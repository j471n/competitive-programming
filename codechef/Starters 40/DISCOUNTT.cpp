...
void solve() {
  readll(n);
  readll(couponPrice);
  readll(couponDiscount);

  vi items(n);

  int totalCostWithoutCoupon = 0;
  int totalCostWithCoupon = couponPrice;

  rep0(i, n) {
    cin >> items[i];
    totalCostWithoutCoupon += items[i];

    if (items[i] >= couponDiscount) {
      totalCostWithCoupon += items[i] - couponDiscount;
    }
  }
